...
final class NBKDoubleWidthTestsOnTextForEachRadixAsUInt256: XCTestCase {
    ...
    override func tearDown() {
        ...
        var encoded = String(self.ascii.drop{  $0 == "0"  })
        if  encoded.isEmpty {
            encoded.append(contentsOf: self.ascii.suffix(1))
        }
        
        NBKAssertDecodingText(decoded, self.radix,          ascii)
        NBKAssertDecodingText(decoded, self.radix,        encoded)
        NBKAssertEncodingText(decoded, self.radix, true,  encoded.uppercased())
        NBKAssertEncodingText(decoded, self.radix, false, encoded.lowercased())
    }
