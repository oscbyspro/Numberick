...
//*============================================================================*
// MARK: * NBK x Resizable Width x Shifts x UIntXR
//*============================================================================*

final class NBKResizableWidthTestsOnShiftsAsUIntXR: XCTestCase {
    ...
    func testBitshiftingLeftByWords() {
        NBKAssertShiftLeft(T(x64:[ 1,  2,  3,  4] as X),   0, T(x64:[ 1,  2,  3,  4] as X))
        NBKAssertShiftLeft(T(x64:[ 1,  2,  3,  4] as X),  64, T(x64:[ 0,  1,  2,  3] as X))
        NBKAssertShiftLeft(T(x64:[ 1,  2,  3,  4] as X), 128, T(x64:[ 0,  0,  1,  2] as X))
        NBKAssertShiftLeft(T(x64:[ 1,  2,  3,  4] as X), 192, T(x64:[ 0,  0,  0,  1] as X))
        NBKAssertShiftLeft(T(x64:[ 1,  2,  3,  4] as X), 256, T(x64:[ 0,  0,  0,  0] as X))
    }
    
    func testBitshiftingLeftByWordsAndBits() {
        NBKAssertShiftLeft(T(x64:[ 1,  2,  3,  4] as X),   3, T(x64:[ 8, 16, 24, 32] as X))
        NBKAssertShiftLeft(T(x64:[ 1,  2,  3,  4] as X),  67, T(x64:[ 0,  8, 16, 24] as X))
        NBKAssertShiftLeft(T(x64:[ 1,  2,  3,  4] as X), 131, T(x64:[ 0,  0,  8, 16] as X))
        NBKAssertShiftLeft(T(x64:[ 1,  2,  3,  4] as X), 195, T(x64:[ 0,  0,  0,  8] as X))
        NBKAssertShiftLeft(T(x64:[ 1,  2,  3,  4] as X), 259, T(x64:[ 0,  0,  0,  0] as X))
    }
    
    func testBitshiftingLeftSuchThatWordsSplit() {
        NBKAssertShiftLeft(T(x64:[~0,  0,  0,  0] as X),   1, T(x64:[~1,  1,  0,  0] as X))
        NBKAssertShiftLeft(T(x64:[ 0, ~0,  0,  0] as X),   1, T(x64:[ 0, ~1,  1,  0] as X))
        NBKAssertShiftLeft(T(x64:[ 0,  0, ~0,  0] as X),   1, T(x64:[ 0,  0, ~1,  1] as X))
        NBKAssertShiftLeft(T(x64:[ 0,  0,  0, ~0] as X),   1, T(x64:[ 0,  0,  0, ~1] as X))
    }
    ...
    func testBitshiftingIsSmart() {
        XCTAssertEqual(T(x64:[1, 2, 3, 4] as X) <<   1, T(x64:[2, 4, 6, 8] as X))
        XCTAssertEqual(T(x64:[1, 2, 3, 4] as X) >>  -1, T(x64:[2, 4, 6, 8] as X))
        
        XCTAssertEqual(T(x64:[1, 2, 3, 4] as X) <<  64, T(x64:[0, 1, 2, 3] as X))
        XCTAssertEqual(T(x64:[1, 2, 3, 4] as X) >> -64, T(x64:[0, 1, 2, 3] as X))
    }
...
//*============================================================================*
// MARK: * NBK x Resizable Width x Shifts x Assertions
//*============================================================================*

private func NBKAssertShiftLeft<T: IntXROrUIntXR>(
...
private func NBKAssertShiftRight<T: IntXROrUIntXR>(
_ lhs: T, _ rhs:  Int, _ result: T,
file: StaticString = #file, line: UInt = #line) {
    //=------------------------------------------=
    let (words, bits) = rhs.quotientAndRemainder(dividingBy: UInt.bitWidth)
